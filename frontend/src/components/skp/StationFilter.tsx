        ...
        <div className="flex items-center justify-between mt-8 mb-12">
            ...
            <div className="flex items-center gap-4">
                ...
                <div className="relative">
                    <button className="bg-primary text-sm py-3 px-6 rounded font-medium text-white" onClick={() => setActive(prev => { return { ...prev, cities: !prev.cities }})}>{filter.city ? filter.city : 'Wybierz miasto'}</button>
                    {active.cities && <ul className="flex flex-col absolute top-[120%] z-10 w-max min-h-max max-h-[1.5in] overflow-y-auto rounded-xl overflow-hidden right-0 shadow-primarySmall">
                        {allFilters.cities.length > 0 ? allFilters.cities.map(city => <li className="py-3 px-6 hover:bg-[#E4E4E4] transition-colors bg-white cursor-pointer" onClick={() => setFilter((prev: {}) => { return { ...prev, city: city}})}>{city}</li>) : <Loader className="mx-auto my-6 w-6" />}
                    </ul>}
                </div>
            </div>
        </div>
